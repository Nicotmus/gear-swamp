...
# カテゴリ絞り込み / ステータス管理 / 自分の名前変更 / 3ヶ月返却目安
# ================================================================

import os, csv, sqlite3, qrcode
...
POST_TYPES = ["試乗希望", "貸して欲しい", "貸します", "譲ります", "雑談"]
...
def init_db():
    with get_conn() as c:
        ...
        CREATE TABLE IF NOT EXISTS items(
            ...
            owner TEXT, location TEXT, note TEXT,
            status TEXT DEFAULT '在庫あり', photo BLOB);
        CREATE TABLE IF NOT EXISTS loans(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            item_id INTEGER, borrower TEXT,
        ...
        CREATE TABLE IF NOT EXISTS reservations(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            item_id INTEGER, reserver TEXT,
            position INTEGER, reserved_date TEXT,
            FOREIGN KEY(item_id) REFERENCES items(id) ON DELETE CASCADE);
        CREATE TABLE IF NOT EXISTS posts(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            author TEXT, ptype TEXT, category TEXT,
            title TEXT, body TEXT, created TEXT);
...
with tab_list:
    ...
    show_arch = st.checkbox("アーカイブ表示", value=False)
    ...
    def list_items():
        with get_conn() as c:
            ...
            if kw:
                ...
                like=f"%{kw}%"; p += [like,like,like]
            if f_cat:
                q+=f" AND category IN ({','.join(['?']*len(f_cat))})"
                p+=f_cat
            if f_owner:
                q+=" AND owner LIKE ?"; p.append(f"%{f_owner}%")
            if f_status:
    ...
    for i,nm,cat,owner,status,photo in list_items():
        with st.container(border=True):
            ...
            # 整備/在庫切替
            new_st=d.selectbox("状態変更",["変更しない","在庫あり","貸出中","整備中","アーカイブ"],key=f"st{i}")
            if d.button("🔧 更新", key=f"upd{i}") and login and is_active(member) and new_st!="変更しない":
                with get_conn() as conn:
            ...
            # アーカイブ
            if x.button("📦 アーカイブ", key=f"arc{i}") and login and is_active(member):
                with get_conn() as conn:
                    conn.execute("UPDATE items SET status='アーカイブ' WHERE id=?", (i,))
                st.rerun()

            # 削除
            confirm=y.checkbox("確認",key=f"cf{i}")
            if z.button
# ================================================================
# app.py --- Gear Swamp（完全版）
# 招待制 / Admin管理 / 在庫・貸出・予約 / 掲示板 / CSV / 写真 /
...
SHARED_PASSCODE = st.secrets.get("passcode", "1234")
INVITE_CODE = st.secrets.get("invite_code", "join-123")
ADMIN_USERS = set(st.secrets.get("admin_users", []))  # ["TETSUYA", ...]
...
# 今回は返却ボタンで必ず在庫ありに戻す運用にしたので、
# 自動繰り上げロジックは使わないが、将来のために残しておいてもOK。
def auto_assign_next(item_id):
    with get_conn() as c:
        first = c.execute(
            "SELECT id, reserver FROM reservations WHERE item_id=? ORDER BY position LIMIT 1",
            (item_id,),
        ).fetchone()
        if not first:
            c.execute("UPDATE items SET status='在庫あり' WHERE id=?", (item_id,))
            return None
        rid, res = first
        today = date.today()
        c.execute(
            """INSERT INTO loans(item_id,borrower,start_date,status)
               VALUES(?,?,?, '貸出中')""",
            (item_id, res, str(today)),
        )
        c.execute("UPDATE items SET status='貸出中' WHERE id=?", (item_id,))
        c.execute("DELETE FROM reservations WHERE id=?", (rid,))
        c.execute(
            "UPDATE reservations SET position=position-1 WHERE item_id=? AND position>1",
            (item_id,),
        )
...
with tab_mem:
    ...
    if not is_admin(member):
        ...
        with col2:
            ...
            to_m = st.selectbox(
                "移管先", [n for n in names if n != from_m], index=0 if len(names) > 1 else 0, key="admin_to"
            )
