...
def upsert_member(name, insta=None, activate=False):
    ...
    r = db_fetchone("SELECT id FROM members WHERE name=%s", (name,))
    if r:
        db_exec(
            "UPDATE members SET insta=%s, is_active=%s WHERE name=%s",
            (insta, bool(activate), name),
        )
    else:
        db_exec(
            "INSERT INTO members(name, insta, is_active, created_at) VALUES(%s,%s,%s,%s)",
            (name, insta, bool(activate), str(date.today())),
        )

def is_active(user):
    if not user:
        return False
    r = db_fetchone("SELECT is_active FROM members WHERE name=%s", (user,))
    return bool(r and bool(r[0]))

def get_insta(user):
    if not user:
        return None
    r = db_fetchone("SELECT insta FROM members WHERE name=%s", (user,))
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as c:
                c.execute("UPDATE members SET name=%s WHERE name=%s", (new_name, old_name))
                c.execute("UPDATE items SET owner=%s WHERE owner=%s", (new_name, old_name))
                c.execute("UPDATE loans SET borrower=%s WHERE borrower=%s", (new_name, old_name))
                c.execute("UPDATE reservations SET reserver=%s WHERE reserver=%s", (new_name, old_name))
                c.execute("UPDATE posts SET author=%s WHERE author=%s", (new_name, old_name))
            conn.commit()
        return True
    except Exception:
        return False
...
def transfer_ownership(frm, to):
    ...
    with get_conn() as conn:
        with conn.cursor() as c:
            c.execute("UPDATE items SET owner=%s WHERE owner=%s", (to, frm))
            cnt = c.rowcount
        conn.commit()
    return cnt
...
def delete_member(member_name):
    with get_conn() as conn:
        with conn.cursor() as c:
            c.execute("DELETE FROM members WHERE name=%s", (member_name,))
            cnt = c.rowcount
        conn.commit()
    return cnt
