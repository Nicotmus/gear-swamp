...
# ================================================================
# タブ構成（在庫登録 / 貸出 / 掲示板 / 履歴 / メンバー / CSV / バックアップ）
# ================================================================
...
# ================================================================
# 在庫登録タブ（カテゴリ・状態だけ MultiSelect 1件選択方式）
# ================================================================
with tab_inv:
 st.subheader("在庫登録")
 c1, c2 = st.columns(2)
 ...
 "状態（1件選択）", ["新品", "美品", "使用感あり", "要整備"],
 default=["新品"], max_selections=1
 )
 ...
 if st.button("登録", disabled=not (login and is_active(member) and name and cat and cond)):
...
with tab_list:
 ...
 if az.button(" 削除", key=f"del{i}") and confirm and login and is_active(member):
 with get_conn() as conn:
